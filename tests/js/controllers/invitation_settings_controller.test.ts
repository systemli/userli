...
function html(invitationsChecked: boolean, limitValue: number): string {
  return `
    <div data-controller="invitation-settings">
      ...
      <div class="hidden" data-invitation-settings-target="waitingPeriodSection">
...
function getTargets(element: HTMLElement) {
  return {
    ...
    waitingPeriodSection: element.querySelector<HTMLElement>(
      "[data-invitation-settings-target='waitingPeriodSection']",
    )!,
  };
}
...
describe("InvitationSettingsController", () => {
  ...
  describe("initial state", () => {
    it("enables auto-toggle and shows limit when invitations on and limit > 0", async () => {
      ...
      const { autoToggle, autoSection, limitSection, waitingPeriodSection } = getTargets(element);
      ...
      expect(waitingPeriodSection.classList.contains("hidden")).toBe(false);
    });
    ...
    it("enables auto-toggle unchecked and hides limit when invitations on and limit = 0", async () => {
      ...
      expect(waitingPeriodSection.classList.contains("hidden")).toBe(true);
    });
  ...
  describe("invitations toggle", () => {
    it("disables auto section and sets limit to 0 when invitations turned off", async () => {
      ...
      const { invitationsToggle, autoToggle, autoSection, limitInput, limitSection, waitingPeriodSection } =
        getTargets(element);
  ...
  describe("auto-invitations toggle", () => {
    it("shows limit input and sets default when auto turned on with limit 0", async () => {
      ...
      const { autoToggle, limitInput, limitSection, waitingPeriodSection } = getTargets(element);
