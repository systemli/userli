 ...
 *   - waitingPeriodSection: The wrapper around the waiting period input (shown/hidden).
 *
 * Values:
...
export default class extends Controller<HTMLElement> {
  ...
  declare waitingPeriodSectionTarget: HTMLElement;
  ...
  connect(): void {
    const currentLimit = Number.parseInt(this.limitInputTarget.value, 10) || 0;
  ...
  invitationsChanged(): void {
    if (!this.invitationsToggleTarget.checked) {
      ...
      const current = Number.parseInt(this.limitInputTarget.value, 10) || 0;
      if (current > 0) {
   ...
   */
  limitChanged(): void {
    const value = Number.parseInt(this.limitInputTarget.value, 10) || 0;
    if (value > 0) {
  ...
  private _sync(): void {
    ...
    // Limit + waiting period: visible only when both toggles are on.
    const showLimit = invitationsEnabled && autoEnabled;
    if (showLimit) {
      this.limitSectionTarget.classList.remove("hidden");
      this.waitingPeriodSectionTarget.classList.remove("hidden");
      this.limitInputTarget.min = "1";
    } else {
      this.limitSectionTarget.classList.add("hidden");
      this.waitingPeriodSectionTarget.classList.add("hidden");
