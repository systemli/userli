...
function auth_userdb_lookup(request)
    ...
    if http_response:status() == 200 then
        ...
        if not(data and data.body and data.body.email and data.body.domain and data.body.quota and data.body.mailCrypt and data.body.mailCryptPublicKey) then
            request:log_error(log_msg['http-ok-malformed'])
            return dovecot.auth.PASSDB_RESULT_INTERNAL_FAILURE, ""
        end

        local attributes = {}
        attributes["user"]                           = data.body.email
        attributes["domain"]                         = data.body.domain
