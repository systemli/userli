...
use AppBundle\Repository\AliasRepository;
use AppBundle\Repository\UserRepository;
use AppBundle\Repository\VoucherRepository;
use AppBundle\Repository\ReservedNameRepository;
use AppBundle\Validator\Constraints\RegistrationConstraint;
...
class RegistrationValidator extends ConstraintValidator
{
    ...
    public function __construct(DomainGuesser $domainGuesser, ObjectManager $manager, $domain)
    {
        ...
        $this->aliasRepository = $manager->getRepository('AppBundle:Alias');
        $this->userRepository = $manager->getRepository('AppBundle:User');
        $this->voucherRepository = $manager->getRepository('AppBundle:Voucher');
        $this->reservedNameRepository = $manager->getRepository('AppBundle:ReservedName');
        $this->domain = $domain;
    }
    ...
    public function validate($value, Constraint $constraint)
    {
        if (!$value instanceof Registration) {
            throw new UnexpectedTypeException('Wrong value type given', 'AppBundle\Form\Model\Registration');
        }
